...
async function getStudentSpecificData(staffId: string) {
  const options = {
    method: "GET",
    headers: {
      accept: "application/json",
...
export default function SpecificStudentPage({ params: { id } }: PropsInterface) {
  const imgURL = process.env.NEXT_PUBLIC_IMG_URL;
  const student = useAppSelector((state) => state.movies.selectedPerson);
  ...
  async function retrieveCharacters(): Promise<SingleCharacterResponse> {
    let { data, error }: SingleCharacterResponse = await getStudentSpecificData(
      id
    );
    return {
      data,
  ...
  return (
    <section className="p-8">
      <div className="flex gap-2 items-center justify-center my-6">
        ...
        <h1 className="text-xl font-bold">Hogwarts Academy: Student Info</h1>
      </div>
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-10 w-full">
          ...
          <div className="sm:col-span-2 space-y-6">
            <h1 className="text-2xl">{student?.actor}</h1>
            <div className="space-y-3">
              <h2 className="text-xl">Hogwarts Academy Info:</h2>
              <p className="capitalize">House: {student?.house || 'Gryffindor'}</p>
              <p className="capitalize">
                Wand:{" "}
                {student?.wand?.core || student?.wand?.wood || student?.wand?.length || "Unicorn Tail-Hair"}
              </p>
            </div>
            <div className="space-y-3">
              <h2>Known For</h2>
